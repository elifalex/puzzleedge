...
import { getPuzzleByIndex, getPuzzleCount } from '../../../src/data/miniSudokuPuzzleLoader';
import { MiniSudokuPuzzle } from '../../../src/constants/types';
...
export default function MiniSudokuPracticePage() {
  ...
  const handleComplete = (time: number) => {
    if (puzzle) {
      const puzzleId = `miniSudoku-${difficulty}-${String(currentIndex + 1000 + (difficulty === 'easy' ? 0 : difficulty === 'medium' ? 1000 : 2000)).padStart(4, '0')}`;
      markPuzzleComplete(difficulty, puzzleId, {
        time,
        hintsUsed: 0,
        completed: true,
    ...
    return (
      <View style={styles.container}>
        <View style={styles.content}>
          <Link href="/games/mini-sudoku/practice" style={styles.backLink}>
  ...
  const puzzleId = `miniSudoku-${difficulty}-${String(currentIndex + 1000 + (difficulty === 'easy' ? 0 : difficulty === 'medium' ? 1000 : 2000)).padStart(4, '0')}`;
  const isCompleted = isPuzzleCompleted(difficulty, puzzleId);
    ...
    <View style={styles.wrapper}>
      <ScrollView ref={scrollViewRef} style={styles.container}>
        <View style={styles.content}>
          ...
          <Text style={styles.title}>Mini Sudoku Practice</Text>
          ...
          <View ref={boardContainerRef}>
            <MiniSudokuBoard key={puzzleId} puzzle={puzzle} mode="practice" onComplete={handleComplete} />
          </View>
